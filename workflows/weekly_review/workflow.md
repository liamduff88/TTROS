...
## Trigger
Weekly schedule fires or Liam requests it. Inputs: the week's open/closed queue items, run_ledger receipts, queue/token_ledger.jsonl window, `business_brain:operating_context/current_priorities.md`.

## Completion contract (default)
- **Done** = one brief artifact covering, in order: shipped (by lane, one line each, linked to receipts/artifacts) · open & blocked (with age and what each waits on: Liam / client / external) · token rollup by lane per §8.5 (est_cost from the deterministic script; non-reporting components listed by name under `unavailable`, never estimated) · signals (lanes trending toward §3.3 promotion math, v0 skill uses, stale Revisit stamps handed to /maintain-os) · proposed top-3 next-week priorities against `business_brain:operating_context/current_priorities.md` for Liam to confirm or reorder. Receipt with its own token block.
