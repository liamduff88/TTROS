...
> Revisit: when ICP (memory/ideal_clients.md), offers, or the LinkedIn lane split changes. · Last touched: 2026-07-08
...
- memory/ideal_clients.md — ICP definition and fit signals (source of truth for matching).
- memory/positioning.md, memory/offers.md — for relevance framing only, not drafting.
- Public LinkedIn/web signals only — no scraping beyond existing legitimate tooling; no purchased lists, no private data.

## Steps
1. **Scope** — confirm N and any stated focus (industry, geography, trigger event). Default geography per memory/ideal_clients.md (Vancouver/Metro Vancouver/BC/Canada, remote NA where practical).
...
4. **Score** — rank by ICP fit strength using memory/ideal_clients.md criteria; select top N with evidence.
...
6. **Output** — artifact: N prospects with name/entity, one-line why-fit, evidence source, ICP score. Receipt with token block. Each entity gets/updates a prospect entity page with sourced facts only.
