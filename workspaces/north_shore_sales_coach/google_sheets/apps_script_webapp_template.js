...
const TEAM_SCORECARD_FORMULA_ROWS = 50;
...
function latestReportDateFormula_() {
  return '=IFERROR(MAX(FILTER({Daily_Team_Summary!A2:A;Daily_Salesperson_Scorecard!A2:A},{Daily_Team_Summary!A2:A;Daily_Salesperson_Scorecard!A2:A}<>"")),TODAY())';
}

function seedDashboardDaily(spreadsheet) {
  const d = latestReportDateFormula_();
  const rows = [
    [d, "Today's Summary", 10, 'As of date', d, 'Daily_Team_Summary,Daily_Salesperson_Scorecard', 'Current report date', 'Start here before the daily floor check.'],
...
function seedTeamScorecard(spreadsheet) {
  const d = latestReportDateFormula_();
  const rows = [];
  for (let rowNumber = 2; rowNumber < TEAM_SCORECARD_FORMULA_ROWS + 2; rowNumber += 1) {
    rows.push(teamScorecardRow_(d, rowNumber));
  }
  return setDashboardRows_(spreadsheet, 'Team_Scorecard', rows);
}

function teamScorecardSalespersonFormula_(rowNumber) {
  return '=LET(names,TOCOL({IFNA(FILTER(Salespeople!B$2:B,Salespeople!B$2:B<>"",(Salespeople!C$2:C=TRUE)+(LOWER(TO_TEXT(Salespeople!C$2:C))="true")+(LOWER(TO_TEXT(Salespeople!C$2:C))="yes")),);IFNA(FILTER(Daily_Salesperson_Scorecard!D$2:D,Daily_Salesperson_Scorecard!A$2:A=$A' + rowNumber + ',Daily_Salesperson_Scorecard!D$2:D<>""),)},1),IFERROR(INDEX(SORT(UNIQUE(FILTER(names,names<>""))),ROW()-1),""))';
}

function teamScorecardRow_(d, rowNumber) {
  const row = String(rowNumber);
  return [
    d,
    teamScorecardSalespersonFormula_(rowNumber),
    '=IF($B' + row + '="","",IFERROR(XLOOKUP($B' + row + ',Salespeople!B:B,Salespeople!C:C,IF(COUNTIFS(Daily_Salesperson_Scorecard!A:A,$A' + row + ',Daily_Salesperson_Scorecard!D:D,$B' + row + ')>0,"synced_scorecard","")),""))',
    '=IF($B' + row + '="","",SUMIFS(Daily_Salesperson_Scorecard!E:E,Daily_Salesperson_Scorecard!A:A,$A' + row + ',Daily_Salesperson_Scorecard!D:D,$B' + row + '))',
    '=IF($B' + row + '="","",SUMIFS(Daily_Salesperson_Scorecard!F:F,Daily_Salesperson_Scorecard!A:A,$A' + row + ',Daily_Salesperson_Scorecard!D:D,$B' + row + '))',
    '=IF($B' + row + '="","",SUMIFS(Daily_Salesperson_Scorecard!G:G,Daily_Salesperson_Scorecard!A:A,$A' + row + ',Daily_Salesperson_Scorecard!D:D,$B' + row + '))',
    '=IF($B' + row + '="","",SUMIFS(Daily_Salesperson_Scorecard!H:H,Daily_Salesperson_Scorecard!A:A,$A' + row + ',Daily_Salesperson_Scorecard!D:D,$B' + row + '))',
    '=IF($B' + row + '="","",SUMIFS(Daily_Salesperson_Scorecard!I:I,Daily_Salesperson_Scorecard!A:A,$A' + row + ',Daily_Salesperson_Scorecard!D:D,$B' + row + '))',
    '=IF($B' + row + '="","",SUMIFS(Daily_Salesperson_Scorecard!J:J,Daily_Salesperson_Scorecard!A:A,$A' + row + ',Daily_Salesperson_Scorecard!D:D,$B' + row + '))',
    '=IF($B' + row + '="","",COUNTIFS(Followups!A:A,$A' + row + ',Followups!D:D,$B' + row + '))',
    '=IF($B' + row + '="","",COUNTIFS(Missing_Data!A:A,$A' + row + ',Missing_Data!D:D,$B' + row + '))',
    '=IF($B' + row + '="","",COUNTIFS(Coaching_Flags!A:A,$A' + row + ',Coaching_Flags!C:C,$B' + row + '))',
    '=IFERROR(G' + row + '/E' + row + ',0)',
    '=IFERROR(H' + row + '/G' + row + ',0)',
    '=IFERROR(I' + row + '/H' + row + ',0)',
    '=IFERROR(J' + row + '/D' + row + ',0)',
    '=IF($B' + row + '="","",AVERAGE(M' + row + ',N' + row + ',O' + row + ',P' + row + ',1-R' + row + '))',
    '=IFERROR(K' + row + '/D' + row + ',0)',
    '=IF($B' + row + '="","",IFERROR(SPARKLINE(FILTER(Daily_Salesperson_Scorecard!E:E,Daily_Salesperson_Scorecard!D:D=$B' + row + ',Daily_Salesperson_Scorecard!A:A>=$A' + row + '-6)),""))',
    '=IF($B' + row + '="","",IFS(D' + row + '=0,"No Update Yet",K' + row + '>0,"Needs Attention",L' + row + '>0,"Watch",Q' + row + '<0.5,"Needs Attention",Q' + row + '<0.7,"Watch",TRUE,"On Track"))',
    '=IF($B' + row + '="","",IFS(T' + row + '="No Update Yet","Check whether this rep needs help logging customer activity.",K' + row + '>0,"Ask for the missing update detail.",L' + row + '>0,"Review coaching flag detail.",Q' + row + '<0.7,"Coach the next sales-process step.",TRUE,"Recognize the complete update and reinforce the next action."))',
  ];
}
...
function seedRepDetail(spreadsheet) {
  ...
  const rows = [
    ...
    [selectedRep, d, 'Selected Salesperson', 20, 'Selected date', d, 'Daily_Team_Summary,Daily_Salesperson_Scorecard', 'Change the selected date to review a prior day.'],
