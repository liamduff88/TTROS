# ANTIGRAVITY.md — Antigravity workbench, TTROS Agentic OS (DORMANT)
> Revisit: on Antigravity's first real task — this file activates then. · Last touched: 2026-07-19.

## Status
Dormant. Per Blueprint V2 §5.4, Antigravity gets a live identity only after it
does its first real task. This file exists so the pattern is fixed before the
tool arrives. Until activation: no agent routes work to Antigravity.

## Pattern on activation (same shape as CLAUDE.md / CODEX.md)
- **Role:** define the specific division of labor that earned the activation —
  one sentence, then keep it to that lane. A workbench, not a department agent.
- **Before working:** `README.md` and `context/PATHS.md` for the repo map;
  `business_brain:index/MEMORY_INDEX.md` before business-context reads;
  ROT.md before layer files.
- **Hard rules:** live workspace (AgenticOSClean) only; no protected paths,
  North Shore files, secrets, or credentials; no external writes or pushes
  without explicit instruction; existing tests stay green.
- **Session/token policy:** unrelated tasks use fresh sessions. Automatic
  compaction must preserve required task context; the shared 500,000-token
  work-item/session fuse is the only token breaker. Report provider-total input,
  fresh input, cached input, output,
  reasoning, and closing context percentage where exposed; otherwise record
  the named field as unavailable, never estimate it.

## Activation checklist
1. First real task completed with a full receipt.
2. Replace this file's contents with the live identity (keep the pattern above).
3. Update AGENTS.md workbench list; stamp both files' Last touched.
4. Add a decisions-log line recording why Antigravity was adopted.
