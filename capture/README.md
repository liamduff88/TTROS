...
> Revisit: before live capture activation or when retention/backup roots change. · Last touched: 2026-08-01.
...
Live Gmail routing also publishes private, content-free dated evidence under
`capture/gmail/YYYY-MM-DD/<scope>/`. That tree is Git-ignored and excluded
from ordinary indexing; the existing typed metadata projection remains the
only search/Graphify route. One poll creates at most one idempotent
`human_review` digest linking those records, and a poll with no material
messages creates no queue item.
